...
@client.event
async def on_ready():
    await client.change_presence(status=discord.Status.online, activity=discord.Game("「사랑해」라는 말로 남을 기만하고, 속이며..."))
...
async def helps (ctx) :
    ...
    embed.add_field(name = "/마키마지배 [멘션]", value = "지정한 대상을 지배하여 어떤 말도 할 수 없게 만듭니다", inline = False)
    embed.add_field(name = "/마키마빵 [멘션]", value = "지정한 대상을 총으로 쏴 보내버립니다", inline = False)
    ...
    embed.add_field(name = "/마키마노래 [링크]", value = "해당하는 유튜브링크의 노래를 재생해줍니다", inline = False)
    ...
    embed.add_field(name = "/마키마운세 [종류: 애정, 재물, 장래, 행운]", value = "마키마가 운세를 봐 줍니다", inline = False)
    embed.add_field(name = "/마키마뽑기 [숫자]", value = "행운의 인물을 뽑습니다", inline = False)
    embed.add_field(name = "/마키마사다리타기 [숫자] [숫자: 리스트]", value = "지정된 횟수 만큼 랜덤한 숫자를 뽑습니다", inline = False)
    await ctx.reply(embed=embed)
